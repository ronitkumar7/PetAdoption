...
class PendingApplicationSerializer(ModelSerializer): 
    status = ChoiceField(['PENDING', 'ACCEPTED', 'DENIED', 'WITHDRAWN'])  
    class Meta:
        model = Application
        fields = '__all__'
        read_only_fields = ['owner', 'applicant', 'email', 'phone1', 'phone2', 'description', 'creation_time', 'last_update_time']

class OtherApplicationSerializer(ModelSerializer): 
    class Meta:
        model = Application
        fields = '__all__'
        read_only_fields = ['owner', 'applicant', 'email', 'phone1', 'phone2', 'description', 'status', 'creation_time', 'last_update_time']
